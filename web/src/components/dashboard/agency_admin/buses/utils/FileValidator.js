...
export class FileValidator {
  static requiredColumns = ['plateNumber', 'busType', 'capacity', 'seatLayout', 'amenities'];
  ...
  static validateFileStructure(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!row.plateNumber || !row.busType || !row.capacity) {
        return {
          isValid: false,
          error: `Row ${i + 1}: Missing required data in plateNumber, busType, or capacity`
        };
      }
  ...
  static processFileData(data) {
    return data.map((row, index) => {
      ...
      return {
        ...
        amenities: (cleanRow.amenities || cleanRow.Amenities || '').split(',').map(a => a.trim()).filter(Boolean),
