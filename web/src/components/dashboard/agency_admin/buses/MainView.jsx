...
import { useAgency } from '../../../../stateManagement/contexts/dashboard/AgencyContext';
import { useDebounce } from '../../../../hooks/useDebounce';

// Import reusable UI components
import { Card, Button, Table, StatusPill, DropdownMenu, SearchBar, FilterDropdown } from '../../../ui';
...
const MainView = ({ setView }) => {
    ...
    }, [debouncedSearch, filters.station, filters.type, filters.status, pagination.currentPage]);
    ...
    const columns = [
        ...
        },
        {
            ...
            Cell: ({ row }) => {
                const items = [
                    { label: 'View Details', icon: <Eye size={16} />, onClick: () => console.log('View bus', row.id) },
                    { label: 'Delete Bus', icon: <Trash2 size={16} />, onClick: () => handleDeleteBus(row.id), isDestructive: true },
                ];
        ...
        <section>
                ...
                <div className="relative">
                    ...
                    {showAddOptions && (
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10 border border-gray-200">
                            <a href="#" onClick={() => handleAddBus('single')} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Single Insert</a>
                            <a href="#" onClick={setView} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Bulk Import</a>
                        </div>
                    )}
                </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <FilterDropdown
                        label="Type"
                        options={['Classic', 'VIP', 'Standard'].map(t => ({ value: t, label: t }))}
                        selected={filters.type}
                        setSelected={(value) => handleFilterChange('type', value)}
                    />
                    <FilterDropdown
                        label="Status"
                        options={['Active', 'Available', 'Under Maintenance', 'Inactive'].map(s => ({ value: s, label: s }))}
            ...
            {isModalOpen && (
                <AddBusModal onClose={() => setIsModalOpen(false)} />
            )}
        </section>
    );
};
